...
//  権限: スペースの所有者(または管理者)のみ
// ============================================================
require_once __DIR__ . '/../_auth.php';
$id = openair_safe_id($_REQUEST['id'] ?? '');   // パストラバーサル対策
require_can_access('spaceData', $id);
?>
<!DOCTYPE html>
<html>
<head>
...
$id = openair_safe_id($_REQUEST['id'] ?? '');
    ...
    list($width1, $height1, $mime, $ext) = openair_prepare_image($tmpName, 'space_image_post.html');
    ...
    if (empty($image1)) {
        openair_upload_error('画像を読み込めませんでした。ファイルが壊れている可能性があります。', 'space_image_post.html');
    }
...
imagecopyresampled($image2, $image1, $startPointX , $startPointY, 0, 0,  $width2, $height2, $width1, $height1);
    ...
    if (!$written || !file_exists($saveTo)) {
        ...
        openair_upload_error('画像の保存に失敗しました。時間をおいて再度お試しください。', 'space_image_post.html');
    }
	...
	echo <<<EOM
			<script type="text/javascript">
			 ...
			 const fileData = {
				  ...
				  db.collection("spaceData").doc(id).set({
				  s_images: firebase.firestore.FieldValue.arrayUnion(fileData)
				  }, {merge: true})
				  .then(() => {
    ...
    && !empty($_FILES['image']['tmp_name']))
{
    ...
    openair_check_upload('image', 'space_image_post.html');
    $tmpName = $_FILES['image']['tmp_name'];

    // 保存先のディレクトリ
    //$dir = __DIR__ . '../../../assets/upload_img/';
	$dir = '../../assets/upload_img/facility/space/';
	...
	echo <<<EOM
			<script type="text/javascript">
			  ...
			  location.href = 'space_image_post.html';
			</script>
			EOM;
}	
?>
