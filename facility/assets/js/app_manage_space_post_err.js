    ...
    <template v-for="item in cat.items" :key="item">
      ...
      <label :for="cat.field + '_' + item">
        {{ item }}
				...
				<tr>
					<th>価格<span class="post-req">*</span></th>
					<td>
<input type="text" id="s_price" name="s_price" v-model="appSpaceData.s_price" class="manage-input" placeholder="5000" >
 <p>※半角数字のみ入力してください（カンマ不要）</p>

    <!-- テキスト表示チェック -->
    <div class="check-btn-area" style="margin-top:10px;">
      <input
        type="checkbox"
        id="price_txt_check"
        v-model="showPriceText"
      >
      <label for="price_txt_check">
        価格をテキストで表示する（設定なし可）
      </label>
    </div>

    <!-- テキスト価格 -->
    <div v-if="showPriceText" style="margin-top:10px;">
      <input
        type="text"
        id="s_price_txt"
        name="s_price_txt"
